...
function get_dotq(P, w, q) result(dq)

real(dp), dimension(4) :: q, dq, quat
real(dp), dimension(3) :: w_W, w
real(dp), dimension(3, 3) :: R, P
...
qn = matrices%dt*get_dotq(P,matrices%w, matrices%q)
...
real(dp) :: dt, k_q(4,4), k_w(3,4), coeffs(4), w(3), &
k_v(3,4), k_x(3,4), R(3,3), wn(3), dw(3), P(3,3)
...
k_q(:, 1) = dt*get_dotq(P,matrices%w, matrices%q)
...
k_q(:, 2) = dt*get_dotq(P,matrices%w+0.5d0*k_w(:,1), matrices%q+0.5d0*k_q(:,1))
...
k_q(:, 3) = dt*get_dotq(P,matrices%w+0.5d0*k_w(:,2), matrices%q+0.5d0*k_q(:,2))
...
k_q(:, 4) = dt*get_dotq(P,matrices%w+k_w(:,3), matrices%q+k_q(:,3))
...
call adaptive_step(matrices)
dt = matrices%dt
if(matrices%E<1d-7) matrices%N = dcmplx(0d0)
...
matrices%vn = matrices%v_CM + euler_3D(real(matrices%F)/mesh%mass, dt)
